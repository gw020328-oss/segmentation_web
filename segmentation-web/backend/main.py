"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  ANTIGRAVITY · 파일 업로드 기반 다중 브랜드 분석 백엔드 v2.0                   ║
║  FastAPI + Gemini API  ·  크롤링 제거, 인코딩 완벽 방어                       ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# ── .env / .env.example 자동 로드 ────────────────────────────────────────────
def _load_dotenv_manual(path: Path) -> None:
    """python-dotenv 없이도 .env 파일을 직접 파싱하여 환경변수로 설정."""
    if not path.exists():
        return
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if "=" in line:
            ...
            if key and not os.environ.get(key):  # 이미 설정된 값은 덮어쓰지 않음
                os.environ[key] = val

_here = Path(__file__).parent
# 우선순위: .env > .env.example
if (_here / ".env").exists():
    _load_dotenv_manual(_here / ".env")
elif (_here / ".env.example").exists():
    _load_dotenv_manual(_here / ".env.example")
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """컬럼명을 text / rating / brand 으로 통일합니다."""
    col_map = {}
...
def safe_read_file(content: bytes, filename: str) -> pd.DataFrame:
    """cp949/utf-8/euc-kr 완벽 방어 + Excel 지원."""
...
# ═════════════════════════════════════════════════════════════════════════════
#  파일 업로드 API  (Multi-CSV / Excel)
# ═════════════════════════════════════════════════════════════════════════════
@app.post("/api/upload")
async def upload_files(files: list[UploadFile] = File(...)):
    """
    여러 CSV/Excel 파일 동시 업로드.
    파일명(확장자 제거) → brand 명으로 자동 지정 후 병합.
    """
    if not files:
    ...
    for f in files:
        ...
        try:
            ...
            # text 컬럼이 없으면 첫 번째 object 컬럼 사용
