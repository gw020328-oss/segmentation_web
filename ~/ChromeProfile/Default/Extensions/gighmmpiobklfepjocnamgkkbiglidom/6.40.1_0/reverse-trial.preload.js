...
const EXPERIMENT_FLAG = "show-reverse-trial-opd";
const OPD_KILL_SWITCH = "reverse-trial-opd-kill-switch";
const TOUCH_POINT_CONFIG_FLAG = "reverse-trial-opd-config";
...
const OPT_OUT_MESSAGE_TYPE = "reverse-trial.opt-out";
const FEEDBACK_MESSAGE_TYPE = "reverse-trial.feedback";
const FEEDBACK_URL = "https://docs.google.com/forms/d/e/1FAIpQLSdMfL5RrrDbPr5Ud1oBm8oYDOFp_TeHLwxKHEGksUY2OMwDCQ/viewform?usp=pp_url";
const TRIAL_DURATION_DAYS = 14;
const MIDPOINT_THRESHOLD_DAYS = 7;
...
async function detectYouTubePremium(timeoutMs = 10000) {
...
function buildDialog(payload) {
    ...
    closeBtn.addEventListener("click", () => {
        void browser_polyfill.runtime.sendMessage({
            type: DIALOG_ACTION_MESSAGE_TYPE,
            touchPointStep: payload.touchPointStep,
            action: "close",
        });
        removeDialog();
    });
    ...
    if (payload.button) {
        ...
        ctaBtn.id = "cta-btn";
        ctaBtn.textContent = payload.button;
        ctaBtn.addEventListener("click", () => {
            ...
            void browser_polyfill.runtime.sendMessage({
                type: DIALOG_ACTION_MESSAGE_TYPE,
                touchPointStep: payload.touchPointStep,
                action: payload.target ? "cta" : "close",
            });
            removeDialog();
        });
    ...
    const linkDefs = [];
    if (payload.learnMore) {
        const learnMore = document.createElement("a");
        learnMore.href = payload.learnMore.url;
        learnMore.target = "_blank";
        learnMore.textContent = payload.learnMore.text;
        ...
        linkDefs.push(learnMore);
    }
    if (payload.dismiss) {
        const dismiss = document.createElement("a");
        dismiss.href = "#";
        dismiss.textContent = payload.dismiss.text;
        ...
        linkDefs.push(dismiss);
    }
    if (payload.optOut) {
        const optOut = document.createElement("a");
        optOut.href = "#";
        optOut.textContent = payload.optOut.text;
        optOut.addEventListener("click", (e) => {
            e.preventDefault();
            void browser_polyfill.runtime.sendMessage({
                type: OPT_OUT_MESSAGE_TYPE,
                touchPointStep: payload.touchPointStep,
            });
            removeDialog();
        });
        linkDefs.push(optOut);
    }
    if (payload.feedback) {
        const feedback = document.createElement("a");
        feedback.href = payload.feedback.url;
        feedback.target = "_blank";
        feedback.textContent = payload.feedback.text;
    ...
    if (linkDefs.length > 0) {
        ...
        for (let i = 0; i < linkDefs.length; i++) {
            linksRow.appendChild(linkDefs[i]);
            if (i < linkDefs.length - 1) {
